...
> **Archive.** Day-of write boundaries for LAN LORDS at HackNYC. Current product documentation is [README.md](README.md). Event file map: [docs/HACKATHON.md](docs/HACKATHON.md).
...
**Starter prompts:** copy/paste from [`README_START_PROMPTS.md`](README_START_PROMPTS.md).
Coder 1: section 4 on `packet/runtime`. Coder 2: section 5 on `packet/core`. Coder 3: section 6 on `packet/integration`.
...
- good candidate: Processor B, affected U.S. Visa/Mastercard traffic at or below $5,000, 17.4% of total traffic
- human approval required
- verification reads actual simulator state

Evidence staging:
1. initial evidence is enough to show global failover violates a hard rule;
2. stale capacity evidence first yields `MORE_EVIDENCE_REQUIRED`;
3. current capacity, affected-cohort, traffic-distribution and rehearsal evidence support the bounded 17.4% candidate;
4. do not reveal every answer on the first pass.
